...
// Las fechas se guardan en UTC pero el negocio opera en hora Argentina: para agrupar
// o filtrar "por día" hay que convertir antes de truncar, si no una venta hecha después
// de las 21:00 (00:00 UTC del día siguiente) se cuenta en el día equivocado.
...
export async function rentabilidad(req: Request, res: Response) {
  ...
  >`
    ...
    WHERE ${fechaLocal('v.fecha')}::date BETWEEN ${desde}::date AND ${hasta}::date AND v.anulada = false
...
export async function ventasPorCanal(req: Request, res: Response) {
  ...
  const rows = await prisma.$queryRaw<{ canal: string; cantidad_ventas: number; total_vendido: number }[]>`
    ...
    FROM ventas WHERE ${fechaLocal('fecha')}::date BETWEEN ${desde}::date AND ${hasta}::date AND anulada = false
    GROUP BY canal
  `
  res.json(rows)
}
...
export async function ventasPorTarjeta(req: Request, res: Response) {
  ...
  >`
    ...
    WHERE vp.medio_pago IN ('debito', 'credito', 'qr')
      AND ${fechaLocal('v.fecha')}::date BETWEEN ${desde}::date AND ${hasta}::date AND v.anulada = false
